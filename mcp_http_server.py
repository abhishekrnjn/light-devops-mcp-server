...
async def list_resources(user: UserPrincipal = Depends(get_current_user)):
    ...
    return {"resources": [resource.dict() for resource in MCP_RESOURCES]}
    ...
    if settings.CEQUENCE_ENABLED:
        try:
            ...
            if resource_path == "logs":
                ...
                # Start the real API call in background (fire and forget for now)
                asyncio.create_task(
                    cequence_client.get_logs(headers=headers, level=level, limit=limit)
                )
                ...
                # Start the real API call in background (fire and forget for now)
                asyncio.create_task(
                    cequence_client.get_metrics(headers=headers, limit=limit)
                )
...
async def get_mcp_resources_tool(
    ...
    try:
        ...
        return {
            ...
            "success": True,
            "result": {"resources": [resource.dict() for resource in MCP_RESOURCES]},
        }
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "mcp_http_server:app", host="0.0.0.0", port=port, reload=True, log_level="info"
    )
