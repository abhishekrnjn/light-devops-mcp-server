...
class CequenceClient:
    ...
    async def _ensure_initialized(self):
        ...
        init_request = {
            ...
            "params": {
                ...
                "clientInfo": {"name": "DevOps-MCP-Server", "version": "1.0.0"},
            },
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"✅ MCP Gateway initialized with session: {self.session_id}"
                )
                ...
                logger.warning(
                    f"⚠️ MCP Gateway initialization failed: {response.status_code}"
                )
                logger.debug(f"Response: {response.text}")
                raise Exception(
                    f"Gateway initialization failed: {response.status_code}"
                )
    ...
    async def _send_initialized_notification(self):
        ...
        response = await self.client.post(
            self.gateway_url, json=initialized_notification, headers=headers
        )
    ...
    async def get_logs(
        ...
        mcp_request = {
            ...
            "id": str(uuid.uuid4()),
            "method": "tools/call",
            "params": {"name": "getMcpResourcesLogs", "arguments": arguments},
        }
        ...
        response = await self.client.post(
            self.gateway_url, json=mcp_request, headers=request_headers
        )
    ...
    async def get_metrics(
        self, headers: Dict[str, str], limit: int = 50, service: Optional[str] = None
    ) -> httpx.Response:
        ...
        mcp_request = {
            ...
            "id": str(uuid.uuid4()),
            "method": "tools/call",
            "params": {"name": "getMcpResourcesMetrics", "arguments": arguments},
        }
    ...
    async def deploy_service(
        self, headers: Dict[str, str], service_name: str, version: str, environment: str
    ) -> httpx.Response:
    ...
    async def rollback_deployment(
        self, headers: Dict[str, str], deployment_id: str, reason: str, environment: str
    ) -> httpx.Response:
    ...
    async def rollback_staging(
        self, headers: Dict[str, str], deployment_id: str, reason: str
    ) -> httpx.Response:
