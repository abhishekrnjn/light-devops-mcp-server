...
class RollbackClient:
    async def rollback_deployment(
        self, deployment_id: str, reason: str, environment: str = "staging"
    ) -> Tuple[Rollback, int, Dict[str, Any]]:
        ...
        if (
            ...
            or "urgent" in reason_lower
            or "security" in reason_lower
        ):
            ...
            status = random.choices(
                ["SUCCESS", "FAILED", "IN_PROGRESS"], weights=[70, 25, 5]
            )[0]
        ...
        enhanced_reason = (
            f"{reason} (Environment: {environment}, Rollback ID: {rollback_id[:8]})"
        )
    ...
    def _get_rollback_message(
        self, status: str, deployment_id: str, environment: str
    ) -> str:
    ...
    async def get_rollbacks(self) -> Tuple[list[Rollback], int, Dict[str, Any]]:
        ...
        reasons = [
            ...
            "Demo rollback for presentation",
            "Experimental feature causing issues",
            "Critical core service failure",
            "Database connection problems",
            "Memory leak in staging",
        ]
        ...
        for i in range(6):  # More rollbacks for better variety
            ...
            if environment == "production":
                if status == "FAILED":
                    status = random.choices(
                        ["SUCCESS", "IN_PROGRESS"], weights=[85, 15]
                    )[0]
            elif environment == "staging":
                if status == "FAILED":
                    status = random.choices(
                        ["SUCCESS", "IN_PROGRESS"], weights=[75, 25]
                    )[0]
        ...
        json_response = {
            ...
            "metadata": {
                ...
                "status_summary": {
                    "success": len(
                        [r for r in sorted_rollbacks if r.status == "SUCCESS"]
                    ),
                    "failed": len(
                        [r for r in sorted_rollbacks if r.status == "FAILED"]
                    ),
                    "in_progress": len(
                        [r for r in sorted_rollbacks if r.status == "IN_PROGRESS"]
                    ),
                },
            },
        }
