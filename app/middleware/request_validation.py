...
class RequestValidationMiddleware(BaseHTTPMiddleware):
    ...
    def _validate_headers(self, request: Request) -> None:
        ...
        for name, value in request.headers.items():
            if self._contains_suspicious_content(value):
                ...
                raise HTTPException(
                    status_code=400, detail="Invalid header content detected"
                )
    ...
    def _validate_query_params(self, request: Request) -> None:
        ...
        for name, value in request.query_params.items():
            ...
            if self._contains_suspicious_content(value):
                ...
                raise HTTPException(
                    status_code=400, detail="Invalid query parameter content detected"
                )
            ...
            try:
                ...
                raise HTTPException(
                    status_code=400, detail="Invalid URL encoding detected"
                )
    ...
    def _validate_url_path(self, request: Request) -> None:
        ...
        if self._contains_suspicious_content(path):
            ...
            raise HTTPException(status_code=400, detail="Invalid URL path detected")
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                if not isinstance(key, str):
                    raise HTTPException(status_code=400, detail="Invalid JSON key type")

                # Check key length
                if len(key) > 1000:
                    raise HTTPException(status_code=400, detail="JSON key too long")
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                if self._contains_suspicious_content(
                    key
                ) or self._contains_suspicious_content(str(value)):
                    raise HTTPException(
                        status_code=400, detail="Suspicious content detected in JSON"
                    )
