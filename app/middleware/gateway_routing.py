...
class GatewayRoutingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        logger.debug(
            f"🔀 Routing request {request.method} {request.url.path} through {self.router_type}"
        )
        ...
        try:
            ...
            response.headers["X-Gateway-Enabled"] = str(
                settings.CEQUENCE_ENABLED
            ).lower()
            ...
            logger.error(
                f"❌ Gateway routing error for {request.method} {request.url.path}: {e}"
            )

            # If Cequence fails, we could potentially retry with direct mode
            if self.router_type == "cequence" and settings.CEQUENCE_ENABLED:
                logger.warning(
                    "🔄 Cequence Gateway failed, consider implementing direct mode fallback"
                )
