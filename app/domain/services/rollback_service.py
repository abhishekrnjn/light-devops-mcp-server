...
class RollbackService:
    ...
    async def rollback(
        ...
        rollback, http_status, json_response = await self.client.rollback_deployment(
    ...
    async def get_recent_rollbacks(self) -> Tuple[list[Rollback], int, Dict[str, Any]]:
        ...
        json_response["message"] = (
            f"Retrieved {len(successful_rollbacks)} successful rollbacks"
        )
        json_response["metadata"]["total_rollbacks"] = len(successful_rollbacks)
        json_response["metadata"]["status_summary"]["success"] = len(
