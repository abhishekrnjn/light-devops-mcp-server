...
class DeployServiceRequest(BaseModel):
    ...
    service_name: str = Field(
        ..., description="Name of the service to deploy", min_length=1
    )
...
class RollbackDeploymentRequest(BaseModel):
    ...
    deployment_id: str = Field(
        ..., description="ID of the deployment to rollback", min_length=1
    )
...
class AuthenticateUserRequest(BaseModel):
    ...
    refresh_token: Optional[str] = Field(
        None, description="Descope refresh token (optional)"
    )
...
class GetMetricsRequest(BaseModel):
    ...
    limit: int = Field(
        50, description="Maximum number of metrics to return", ge=1, le=1000
    )
    service: Optional[str] = Field(None, description="Filter by service name")
...
class ResourceRequest(BaseModel):
    ...
    limit: int = Field(
        100, description="Maximum number of items to return", ge=1, le=1000
    )
...
class MetricsResourceRequest(ResourceRequest):
    ...
    time_range: Optional[str] = Field(
        None, description="Time range for metrics (e.g., '1h', '24h', '7d')"
    )
    ...
    def validate_time_range(cls, v):
        if v is not None:
            ...
            if v not in allowed_ranges:
                raise ValueError(
                    f'Time range must be one of: {", ".join(allowed_ranges)}'
                )
        return v
